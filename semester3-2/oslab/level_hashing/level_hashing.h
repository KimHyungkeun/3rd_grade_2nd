...
typedef struct level_hash {               // A Level hash table : 레벨해쉬 테이블
    level_bucket *buckets[2];             // The top level and bottom level in the Level hash table : 레벨해쉬 테이블에 존재하는 toplevel과 bottomlevel
    uint64_t level_item_num[2];           // The numbers of items stored in the top and bottom levels respectively : toplevel과 bottomlevel에 각각 저장된 아이템 갯수
    uint64_t addr_capacity;               // The number of buckets in the top level : top level에 있는 bucket 갯수
    uint64_t total_capacity;              // The number of all buckets in the Level hash table : level hash table에 있는 모든 bucket 갯수
